...
const List = (list: tList) => {
    ...
    return (
        <>
            <div className="p-3 w-96 h-max rounded-lg space-y-4 shadow bg-gray-900 text-slate-400 border border-[#101204] hover:border-slate-700 transition-all duration-50">
                ...
                <div className="h-5 hover:h-10 transition-all duration-200 footer text-slate-400  w-full flex justify-center" onMouseOver={() => setShowAddButton(true)} onMouseLeave={() => setShowAddButton(false)}>
                    ...
                    {showAddButton && <button onClick={() => store.addNewCardtoList(list.id)} className="text-lg rounded-[50%]  hover:text-white flex self-center bg-gray-900 hover:bg-[#242424] font-semibold p-2 outline-1 active:outline outline-[#646cff] ">
                        <svg fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6">
